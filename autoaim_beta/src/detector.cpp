...
#endif
        if (this->update_our_color_if_first_detected(armor_name_string)) armor_name_string = "friend_" + armor_name_string;
...
bool Net_Detector::if_is_gray(const cv::Mat& img_bgr, std::vector<cv::Point2f>& big_rec) const 
{
    ...
    std::vector<cv::Point2f> dst_points;
    dst_points.push_back(cv::Point2f(0, 0));
    dst_points.push_back(cv::Point2f(32 - 1, 0));
    dst_points.push_back(cv::Point2f(32 - 1, 32 - 1));
    dst_points.push_back(cv::Point2f(0, 32 - 1));
