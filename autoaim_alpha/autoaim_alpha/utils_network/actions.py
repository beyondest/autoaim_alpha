...
def trans_logits_in_batch_to_result(logits_in_batch:Union[torch.Tensor,np.ndarray],classes_nums:Union[int,None] = None)->tuple:
    ...
    if isinstance(logits_in_batch,np.ndarray):
        if logits_in_batch.ndim == 1 and classes_nums is not None:
            logits_in_batch = logits_in_batch.reshape(classes_nums,-1)
